...
const INK_CAPABILITY_BY_KEY = {
  BK: 'measure_bk_level',
  C: 'measure_c_level',
  M: 'measure_m_level',
  PGBK: 'measure_pgbk_level',
  Y: 'measure_y_level',
};
...
async function detectCanonPrinter(ip, options = {}) {
  ...
  try {
    ...
    return buildDetectionResult(ip, 1, response.data);
  } catch (method1Error) {
    try {
      ...
      return buildDetectionResult(ip, 2, modelResponse.data, statusPageResponse.data);
    } catch (method2Error) {
...
function buildDetectionResult(ip, method, modelJs, statusPageHtml = '') {
  const capabilityInfo = parseCapabilityInfo(modelJs, statusPageHtml);

  return {
    ip,
    method,
    capabilities: capabilityInfo.capabilities,
    capabilitiesOptions: capabilityInfo.capabilitiesOptions,
  };
}

function parseCapabilityInfo(modelJs, statusPageHtml) {
  const inks = parseInkKeys(modelJs);
  const capabilities = inks
    .map((inkKey) => INK_CAPABILITY_BY_KEY[inkKey])
    .filter(Boolean);

  if (hasSignalInfo(modelJs, statusPageHtml)) {
    capabilities.push('measure_signal_strength');
  }

  capabilities.push('alarm_running', 'alarm_printer_error');
...
function parseInkKeys(modelJs) {
  const colorOrder = parseInkColorOrder(modelJs);
  const inkRegex = /inktank\[\d+\]=\[(\d+),(\d+),(\d+)\];/g;
  const inkKeys = [];
  let match;

  while ((match = inkRegex.exec(modelJs)) !== null) {
    const [, colorId] = match.map(Number);
    const inkKey = colorOrder[colorId];

    if (inkKey && !inkKeys.includes(inkKey)) {
      inkKeys.push(inkKey);
    }
  }
...
function parseInkColorOrder(modelJs) {
  const match = modelJs.match(/var\s+inkCOL\s*=\s*\[([^\]]+)\];/i);
  if (!match) {
    return [];
  }
...
function hasSignalInfo(modelJs, statusPageHtml) {
  if (/g_signal_strength\s*=\s*'\d+'/i.test(modelJs)) {
    return true;
  }

  if (/g_link_quality\s*=\s*'\d+'/i.test(modelJs)) {
    return true;
  }

  return /Signal Strength/i.test(statusPageHtml);
}
